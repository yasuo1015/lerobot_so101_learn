...
python -m lerobot.async_inference.robot_client \
  ...
  --robot.id=<YOUR_FOLLOWER_ID> \                        # 你的从动臂 ID，自定义名称即可
  --robot.cameras='{ base_0_rgb: {type: opencv, index_or_path: "<YOUR_BASE_CAMERA_PATH>", width: 640, height: 480, fps: 30, fourcc: "MJPG"} }' \  # 相机路径
  --server_address=127.0.0.1:18080 \               
  --policy_type=pi0 \                                 
  --pretrained_name_or_path=<YOUR_PI0_MODEL_PATH> \      # 你的模型路径，可以是本地路径或 Hugging Face 模型名
  --policy_device=cuda \                        
  --client_device=cpu \                         
  --actions_per_chunk=50 \                               # 每次返回多少步动作，值越大越平滑，但响应更慢
  --chunk_size_threshold=0.6 \                           # 动作队列剩余多少比例时开始请求下一轮推理
  --fps=30 \                             
  --task="<YOUR_TASK_DESCRIPTION>"                       # 任务描述，建议和训练时保持一致
```
